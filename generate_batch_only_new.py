...
OUTPUT_BASE = "outputs_by_type_style_v2"  # new folder to avoid overwriting
MAX_SAMPLES = 1500  # process first 1500 only
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
TRIM_SECONDS = 2  # trim first 2 seconds
SAMPLERATE = 16000  # Hz
...
# Pick one default embedding (can change speaker ID for variety)
...
for idx, row in df.iterrows():
    ...
    try:
       ...
       # --- Trim first 2 seconds ---
            trim_samples = TRIM_SECONDS * SAMPLERATE
            if speech.numel() > trim_samples:
                speech = speech[trim_samples:]

        # --- Normalize waveform ---
            speech = speech / speech.abs().max()

        sf.write(file_path, speech.cpu().numpy(), SAMPLERATE)
